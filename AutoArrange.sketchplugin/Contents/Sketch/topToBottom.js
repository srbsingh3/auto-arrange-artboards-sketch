...
var myFunction = function(givenArtboard) {
  for (i = 0; i < artboards.length; i++) {
    if (
      (Math.abs(givenArtboard.frame.x - artboards[i].frame.x) < givenArtboard.frame.width) &&
      (Math.abs(givenArtboard.frame.x - artboards[i].frame.x) < artboards[i].frame.width)
      // && (Math.abs(givenArtboard.frame.y - artboards[i].frame.y) != 0)
    ) {
      artboards[i].frame.x = givenArtboard.frame.x;
    }
  }
}
...
sketch.UI.message('Artboards in your layer-list have been sorted vertically.');
